...
function Torrent(id,
  ...
  if (this.seedsConnected > 3) {
    this.availability = 2;
  }

  if (this.seedsConnected >= 1 && this.seedsConnected <= 3) {
    this.availability = 1;
  }
...
Torrent.prototype.isStatusError = function () { 
  console.log(_.isEmpty(this.error));
...
Torrent.prototype.hasSeeds = function () {
  ...
  if (this.percent == 0 && this.isStatusDownloading() && this.seedsConnected == 0 && this.peersConnected < 2 && !this.isPrivate && moment().diff(moment(this.dateAdded), 'minutes') > 5) {
    return false;
  }
  //Give extra time for private torrents
  if (this.percent == 0 && this.isStatusDownloading() && this.seedsConnected == 0 && this.peersConnected < 2 && this.isPrivate && moment().diff(moment(this.dateAdded), 'minutes') > 10) {
    return false;
  }
